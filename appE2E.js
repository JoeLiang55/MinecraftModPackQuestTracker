/*
  Enigmatica 2: Expert – Quest Tracker
  Based on the Nomifactory tracker (app.js) but configured for E2E.
  Reads defaultquests/DefaultQuestsE2E.json (BetterQuesting NBT-style JSON).
*/

// ── Chapter order for sidebar ────────────────────────────────────────
...
var CHAPTER_NAME_ALIASES = {
  'environmental tech': 'enviromental tech',
  'bragging right': 'bragging rights'
};
...
// ── Load quest data from repo ────────────────────────────────────────
function loadQuestFileFromUrl() {
  fetch('defaultquests/DefaultQuestsE2E.json')
    .then(function (r) { return r.ok ? r.json() : Promise.reject(new Error('HTTP ' + r.status)); })
    .then(function (data) {
      questData = data;
      console.log('E2E quest data loaded');
      tryMergeAndRender();
    })
    .catch(function (err) {
      showError('Failed to load DefaultQuestsE2E.json: ' + err.message);
    });
}
...
// ── Player name resolution (UUID → Mojang username) ──────────────────
function tryResolvePlayerName(filename) {
  if (!filename) return;
  var uuidRegex = /([0-9a-f]{8}-?[0-9a-f]{4}-?[0-9a-f]{4}-?[0-9a-f]{4}-?[0-9a-f]{12})/i;
  var match = filename.match(uuidRegex);
  if (!match) return;
  var uuid = match[1].replace(/-/g, '');
  showPlayerName(match[1], null);
  fetch('https://api.ashcon.app/mojang/v2/user/' + uuid)
    .then(function (r) { return r.ok ? r.json() : Promise.reject(new Error(r.status)); })
    .then(function (data) { if (data && data.username) showPlayerName(data.username, uuid); })
    .catch(function () { console.log('Could not resolve username for UUID:', uuid); });
}
...
// ── Player file upload (JSON or NBT .dat) ────────────────────────────
    ...
    try {
      ...
      try {
        ...
        if (text.trim().startsWith('{')) {
          ...
          console.log('Player data loaded as JSON');
          tryMergeAndRender();
          return;
        }
      } catch (_) { /* not JSON */ }
      playerData = await parseNBT(arrayBuffer);
      console.log('Player data loaded from NBT');
      tryMergeAndRender();
    } catch (error) {
      showError('Error parsing PlayerData: ' + error.message);
      console.error(error);
    }
  };
  reader.onerror = function () { showError('Error reading PlayerData file'); };
...
async function parseNBT(arrayBuffer) {
  ...
  if (data[0] === 0x1f && data[1] === 0x8b) {
    try { data = pako.inflate(data); } catch (e) { throw new Error('Failed to decompress gzip: ' + e.message); }
  }
  ...
  function readLong() { const h = view.getInt32(offset, false); const l = view.getUint32(offset + 4, false); offset += 8; return h * 0x100000000 + l; }
  ...
  function readString() { const len = readShort(); const bytes = data.slice(offset, offset + len); offset += len; return new TextDecoder('utf-8').decode(bytes); }
  function readByteArray() { const len = readInt(); const a = Array.from(data.slice(offset, offset + len)); offset += len; return a; }
  function readIntArray() { const len = readInt(); const a = []; for (let i = 0; i < len; i++) a.push(readInt()); return a; }
  function readLongArray() { const len = readInt(); const a = []; for (let i = 0; i < len; i++) a.push(readLong()); return a; }
  function readTag(tagType) {
    switch (tagType) {
      ...
      case 9: { const lt = readByte(); const len = readInt(); const list = []; for (let i = 0; i < len; i++) list.push(readTag(lt)); return list; }
      case 10: { const compound = {}; while (true) { const type = readByte(); if (type === 0) break; const name = readString(); compound[name] = readTag(type); } return compound; }
  ...
  if (rootType !== 10) throw new Error('NBT must start with compound tag');
...
function extractChapters(root) {
  ...
  // Sort chapters to match ORDERED_CHAPTER_NAMES
  function norm(n) { return (n || '').trim().toLowerCase(); }
  function aliased(name) { var n = norm(name); return CHAPTER_NAME_ALIASES[n] || n; }
  ...
  ORDERED_CHAPTER_NAMES.forEach(function (want) {
    ...
    for (var i = 1; i < chapters.length; i++) {
      ...
      if (aliased(chapters[i].name) === wantNorm) { ordered.push(chapters[i]); used.add(i); break; }
    }
  });
  for (var j = 1; j < chapters.length; j++) { if (!used.has(j)) ordered.push(chapters[j]); }
  chapters = ordered;
  console.log('E2E chapters:', chapters.length);
}
...
function checkForSharedProgress() {
  ...
  if (sharedPlayerName) showPlayerName(sharedPlayerName, null);
